...
@metacmd(command='runs')
def runs(update: Update, context: CallbackContext):
    update.effective_message.reply_text(random.choice(fun_strings.RUN_STRINGS))

@metacmd(command='slap')
def slap(update: Update, context: CallbackContext):
    ...
    if user_id == bot.id:
        temp = random.choice(fun_strings.SLAP_Meta_TEMPLATES)
...
@metacmd(command='roll')
def roll(update: Update, context: CallbackContext):
    update.message.reply_text(random.choice(range(1, 7)))

@metacmd(command='toss')
def toss(update: Update, context: CallbackContext):
    update.message.reply_text(random.choice(fun_strings.TOSS))

@metacmd(command='shrug')
...
def insult(update: Update, context: CallbackContext):
    ...
    reply_text(random.choice(fun_strings.INSULTS_STRINGS))    

@metacmd(command='abuse')
