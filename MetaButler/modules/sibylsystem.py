...
def sibyl_ban(update: Update, context: CallbackContext):
    ...
    if client:
        ...
        if data.banned:
            ...
            txt += "Target was Eliminated with <b>{}</b>\n\n".format("Lethal Eliminator" if not data.is_bot else "Destroy Decomposer")
            ...
            txt += "<i>If the enforcement was unjust in any way, kindly report it to @PublicSafetyBureau or disable this feature using /sibylban</i>"
...
dispatcher.add_handler(CommandHandler(command="sibylban", callback=toggle_sibyl, run_async=True, filters=Filters.chat_type.groups), group=100)