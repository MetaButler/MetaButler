from MetaButler.modules.helper_funcs.telethn import HIGHER_AUTH, telethn
from MetaButler import  WHITELIST_USERS
from telethon.tl.types import ChannelParticipantsAdmins


async def user_is_ban_protected(user_id: int, message):
    status = False
    if message.is_private or user_id in (HIGHER_AUTH +  WHITELIST_USERS):
        return True
...
async def MetaButler_is_admin(chat_id: int):
    status = False
    MetaButler = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if MetaButler.id == user.id:
            status = True
            break
    return status
